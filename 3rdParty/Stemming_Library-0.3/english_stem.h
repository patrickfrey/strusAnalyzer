...
namespace stemming
	...
	class english_stem : stem<Tchar_type,Tchar_traits>
		{	
	public:
		typedef stem<Tchar_type,Tchar_traits> Parent;

		english_stem() : m_first_vowel(0)
			{}
		//---------------------------------------------
...
#undef EITHER_LOOSE
		...
		void step_2(std::basic_string<Tchar_type, Tchar_traits>& text)
			...
			else if (Parent::m_r1 <= static_cast<int>(text.length()-3) &&
			...
			else if (Parent::m_r1 <= static_cast<int>(text.length()-2) &&
		...
		void step_5(std::basic_string<Tchar_type, Tchar_traits>& text)
			...
			if (is_either(text[text.length()-1], 'e', 'E') )
				...
				else if (Parent::m_r1 != text.length() &&
			...
			else if (Parent::m_r2 != text.length() &&
